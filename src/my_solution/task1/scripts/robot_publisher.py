...
def current_goal_is_met(robot_position):
    ...
    if((robot_initial >= initial_point_final)):
        if current_index_target == 6:
            ...
            return [True,desired_angular_vel ]
    else:
        return [False, 0.0]
...
def act(robot_vel_publisher,  robot_position, position_robot, right_obstacle, left_obstacle, front_obstacle, move_flag):
    ...
    if move_flag[0] and move_flag[1] == 0.0 :
        #stop 
        stop_robot(robot_vel, robot_vel_publisher)
    elif move_flag[0] and move_flag[1] != 0.0:
        #stop 
        stop_robot(robot_vel, robot_vel_publisher)
        ...
        #turn left
        robot_vel.linear.x = 0.1
        robot_vel.angular.z = move_flag[1]
        robot_vel_publisher.publish(robot_vel) 
        rospy.sleep(0.05)
        stop_robot(robot_vel, robot_vel_publisher)
        #continue moving
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        move_flag = current_goal_is_met(robot_position)
        
        message = act(robot_vel_publisher,  robot_position, position_robot, right_obstacle, left_obstacle, front_obstacle, move_flag)
